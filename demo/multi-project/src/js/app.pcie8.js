...
$.extend({
    ...
    request: function (obj, postType = 'form') {
        // console.log(obj)
        var data = obj.data || '',
            type = obj.method || 'get',
            success = obj.success,
 ...
 */
    ...
    getParam: function (key) {
        var param = {};
        location.search.substr(1).split('&').map(function (i) {
            return i.split('=')
        }).forEach(function (i) {
            param[i[0]] = i[1]
        });
    ...
    render: function (id, data) {
        var view = $('#' + id);
        var isSaved = !!view.data('template');
        if (!isSaved) {
            view.data('template', view.html());
        }
        var templateStr = isSaved ? view.data('template') : view.html();
        layui && layui.laytpl(templateStr).render(
            data,
            function (html) {
                view.html(html);
                view.css('display') === 'none' && view.css('display', '');
            }
        );
    },
    // 触底加载
